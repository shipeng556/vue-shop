<template>
  <div>
      ...
      >
        ...
        <template slot="opt" slot-scope="scope">
          <el-button
            ...
            class="el-icon-edit"
            size="mini"
            @click="showEidtCateDialog(scope.row.id)"
          >编辑</el-button>
          <el-button
            type="primary"
    ...
    <el-dialog
      ...
      width="50%"
      @close="eidtCateDialogClosed"
    >
      <el-form
        :model="eidtCateForm"
        :rules="eidtCateFormRule"
        ref="eidtCateFormRef"
        label-width="70px"
      >
...
export default {
  ...
  methods: {
    ...
    // 展示编辑分类查询的对话框
    async showEidtCateDialog(id) {
      const { data: res } = await this.$http.get('categories/' + id)

      if (res.meta.status !== 200) {
        return this.$message.error('查询分类信息失败！')
      }
      this.eidtCateForm = res.data
    ...
    eidtCateInfo() {
      this.$refs.eidtCateFormRef.validate(async valid => {
        ...
        const { data: res } = await this.$http.put(
          'categories/' + this.eidtCateForm.id,
          {
            cat_name: this.eidtCateForm.cat_name
          }
        )
