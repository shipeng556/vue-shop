<template>
  <div>
    ...
    <!-- 修改用户的对话框 -->
    <el-dialog title="修改用户" :visible.sync="eidtDialogVisible" width="30%" @close="eidtDialogClosed">
      <el-form :model="eidtForm" :rules="eidtFormRule" ref="eidtFormRef" label-width="70px">
  ...
  methods: {
    ...
    async userStateChange(useInfo) {
      ...
      const { data: res } = await this.$http.put(
        `users/${useInfo.id}/state/${useInfo.mg_state}`
      )
      if (res.meta.status !== 200) {
